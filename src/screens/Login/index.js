...
import { loginUser } from "../../actions/AuthAction"; //ditambahkan dari AuthAction

const Login = ({ navigation }) => {
  // dideklarasikan beberapa state
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showAlert, setShowAlert] = useState(false);
  const [alertMessage, setAlertMessage] = useState("");

  // dideklrasikan beberapa fungsi
  ...
  return (
    <Box flex={1} backgroundColor="$blue400" justifyContent="center">
      ...
      >
        ...
        <FormControl>
          {/* nilai onChangeText dan value diubah */}
          <Input
            label={"Login"}
            width={"$full"}
            height={"$10"}
