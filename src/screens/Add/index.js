...
import { addNote, getNote } from "../../actions/AuthAction"; //ditambahkan
...
const Add = ({ navigation }) => {
  ...
  // fungsi ditambahkan
  // Fungsi untuk menampilkan atau menyembunyikan alert
  ...
  const onAddNote = async () => {
    if (title && content && status && category) {
      ...
      console.log(data);
      try {
        // Panggil fungsi addNote dari AuthAction
    ...
    <ScrollView>
      <Box flex={1} backgroundColor="$white">
        <Box shadowColor="$black" shadowOffset={{ width: 0, height: 2 }} shadowOpacity={"$25"} shadowRadius={"$3.5"} elevation={"$5"} backgroundColor="$white" borderRadius={"$md"} mt={"$8"} mx={"$3"} px={"$3"} pt={"$2"}>
          ...
          <FormControl>
            <Input label={"Title"} width={"$full"} height={"$9"} onChangeText={(title) => setTitle(title)} />
            ...
            {/* properti onPress diarahkan ke fungsi onAddNote */}
        ...
        <Modal isOpen={isModalVisible} onClose={toggleModal} finalFocusRef={this.btnRef}>
          <ModalBackdrop />
          <ModalContent backgroundColor="$white" padding={"$1"} borderRadius={"$lg"}>
            <ModalHeader>
              <VStack space="sm">
            ...
            <ModalFooter>
              <Box flex={1} flexDirection="column" justifyContent="space-evenly">
                <Pressable
                  ...
                  borderRadius={"$sm"}
                  alignItems="center"
                  // diubah, tombol mengarah ke fungsi handlerAddCategory
